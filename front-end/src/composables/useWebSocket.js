...
export function useWebSocket(baseUrl, endpoint = "data") {
  ...
  async function connect(symbol, options = { days: 90 }) {
    ...
    try {
      ...
      console.log(
        `Attempt ${connectionAttempts}: Connecting to ${endpoint} WebSocket for ${symbol}USDT...`
      );

      // Create new WebSocket connection
      // Use the provided endpoint (data or raw-data)
      const url = `${baseUrl}/ws/${endpoint}?ticker=${symbol}USDT&days=${options.days}`;
      socket = new WebSocket(url);

      // Configure event handlers
      socket.onopen = () => {
        console.log(
          `${endpoint} WebSocket connection established successfully!`
        );
        isConnecting = false;
      ...
      socket.onmessage = (event) => {
        try {
          ...
          if (Array.isArray(json) && json.length > 0) {
            console.log(
              `Received ${json.length} ${endpoint} records via WebSocket`
            );
            data.value = json;
            isLoading.value = false; // Only disable loading when data arrives
          } else {
            console.warn(
              `Received unexpected ${endpoint} WebSocket data format:`,
              json
            );
          }
        } catch (parseError) {
          console.error(
            `Error parsing ${endpoint} WebSocket data:`,
            parseError
          );
      ...
      socket.onclose = (event) => {
        console.log(
          `${endpoint} WebSocket closed: ${event.code} ${event.reason}`
        );
        isConnecting = false;
        ...
        reconnectTimeout = setTimeout(() => connect(symbol, options), 1000);
        ...
        if (
          ...
          console.log(
            `Attempting to reconnect (Attempt number: ${connectionAttempts}...)`
          );
      ...
      // Add data timeout
      setTimeout(() => {
        if (isLoading.value && symbol === currentSymbol && !fallbackUsed) {
          console.log(`Timeout waiting for ${endpoint} WebSocket data`);
          fallbackToREST(symbol, options);
        }
      }, 5000);
    } catch (err) {
      ...
      fallbackUsed === false ? connectionAttempts++ : (connectionAttempts = 0);
      error.value = err;
  ...
  async function fallbackToREST(symbol, options = { days: 90 }) {
    ...
    try {
      ...
      console.error(
        `Fallback API request for ${endpoint} also failed:`,
        fallbackError
      );
